...
from katana_tonestream.llm_generator import PatchGenerationError, generate_patch
...
@patch("litellm.completion")
def test_character_analysis_feeds_types_and_values_phases(mock_completion):
    mock_completion.side_effect = _happy_side_effect()
    generate_patch("A", "B", "", "")
    types_prompt = mock_completion.call_args_list[1].kwargs["messages"][-1]["content"]
    values_prompt = mock_completion.call_args_list[2].kwargs["messages"][-1]["content"]
    assert "scooped mids" in types_prompt
    assert "scooped mids" in values_prompt
